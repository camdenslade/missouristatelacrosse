// src/Women/Local/Admin/Tabs/EmailCenter.jsx
import {
    addDoc,
    collection,
    deleteDoc,
...
export default function EmailCenter(){
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      try {
        const snap = await getDocs(collection(db, "groupsw"));
  ...
  const handleAddGroup = async () => {
    ...
    try{
      const docRef = await addDoc(collection(db, "groupsw"), {
        name: newGroupName,
        members: [],
        createdAt: new Date(),
      });
  ...
  const handleAddEmail = async () => {
    ...
    await updateDoc(doc(db, "groupsw", selectedGroup), { members: updated });
    dispatch({
      type: "UPDATE_GROUP",
      id: selectedGroup,
  ...
  const handleDeleteGroup = async (id) => {
    await deleteDoc(doc(db, "groupsw", id));
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white p-6 rounded shadow animate-fadeIn">
      <h2 className="text-2xl font-bold mb-4">Email Center — Women’s Program</h2>
