// src/Women/Local/Admin/Tabs/AccountRequests.jsx
...
export default function ManageAccountRequests(){
  ...
  const fetchRequests = async () => {
    try{
      ...
      const data = await apiRequest(`/api/account-requests?program=${program}`);
  ...
  const approveRequest = async (id) => {
    ...
    try{
      const text = await apiRequest(`/api/account-requests/${id}/approve?program=${program}`, {
        method: "POST",
        parseAs: "text",
      });
      dispatch({
  ...
  const rejectRequest = async (id) => {
    ...
    dispatch({ type: "REMOVE_REQUEST", id });
    try{
      const text = await apiRequest(`/api/account-requests/${id}?program=${program}`, {
        method: "DELETE",
        parseAs: "text",
      });
      dispatch({
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 p-6 bg-white shadow rounded animate-fadeIn">
      <h1 className="text-2xl font-bold mb-4">
        Pending Account Requests — <span className="text-[#5E0009]">Women</span> Program
      </h1>
