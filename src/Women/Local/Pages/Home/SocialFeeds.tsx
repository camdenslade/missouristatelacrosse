...
export default function SocialFeeds(){
    ...
    const fetchPosts = useCallback(async (isBackground = false) => {
        try{
            ...
            const snap = await apiRequest<ApiInstagramFeed>("/api/site-content/instagramFeedw");
            if (!snap?.data){
    ...
    const handleAddPost = useCallback(async () => {
        ...
        try{
        const snap = await apiRequest<ApiInstagramFeed>("/api/site-content/instagramFeedw").catch(() => null);
        const existing = snap?.data?.posts || [];

        const updated = Array.isArray(existing)
            ? [...existing, newUrl.trim()]
            : [newUrl.trim()];
        await apiRequest("/api/site-content/instagramFeedw", {
          method: "PUT",
          json: { posts: updated },
        });
    ...
    useEffect(() => {
        const cached: { posts?: string[]; ts?: number } =
          JSON.parse(localStorage.getItem("instagramFeedw") || "{}");
