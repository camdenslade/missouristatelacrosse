...
export default function SponsorForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const stored = JSON.parse(localStorage.getItem("sponsorSubmissionsw") || "[]");
      stored.push({ ...form, timestamp: new Date().toISOString() });
      localStorage.setItem("sponsorSubmissionsw", JSON.stringify(stored));
    } catch (err) {
  ...
  return (
    <section className="bg-white py-20 text-center px-6 relative">
      ...
      <p className="max-w-2xl mx-auto text-gray-700 mb-8 text-lg">
        Partner with Missouri State Women's Lacrosse through sponsorships and community
