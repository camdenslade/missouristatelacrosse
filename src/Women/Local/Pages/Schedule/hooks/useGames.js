...
export default function useGames(){
  ...
  const fetchGames = async () => {
    try {
      ...
      const snapshot = await getDocs(collection(db, "gamesw"));
      ...
      localStorage.setItem("gamesw", JSON.stringify(sorted));
      return sorted;
    } catch (err){
  ...
  const saveGame = async (formData, editingId) => {
    try{
      ...
      if (editingId) await updateDoc(doc(db, "gamesw", editingId), gameData);
      else await addDoc(collection(db, "gamesw"), gameData);
  ...
  const removeGame = async (id) => {
    try {
      await deleteDoc(doc(db, "gamesw", id));
