...
export default function useCoaches(){
  ...
  const fetchCoaches = useCallback(async () => {
    ...
    try{
      const snapshot = await getDocs(collection(db, "coachesw"));
  ...
  const uploadPhoto = useCallback(async (file) => {
    ...
    try{
      ...
      const storageRef = ref(storage, `coachesw/${fileName}`);
  ...
  const saveCoach = useCallback(
    async (formData, editingId = null) => {
      try{
        ...
        if (editingId) {
          await updateDoc(doc(db, "coachesw", editingId), coachData);
        } else {
          await addDoc(collection(db, "coachesw"), coachData);
        }
  ...
  const removeCoach = useCallback(
    async (id) => {
      try{
        await deleteDoc(doc(db, "coachesw", id));
        dispatch({
          type: "SET_COACHES",
