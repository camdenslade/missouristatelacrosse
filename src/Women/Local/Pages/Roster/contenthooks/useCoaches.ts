...
type CoachesAction =
  | { type: "SET_COACHES"; payload: CoachApi[] }
...
function reducer(state: CoachesState, action: CoachesAction): CoachesState {
  switch (action.type){
    case "SET_COACHES":
...
function parseCoachData(coach: CoachApi | null): Record<string, JsonValue> {
  if (!coach) return {};
  if (coach.data && typeof coach.data === "string"){
    try{
      return JSON.parse(coach.data) as Record<string, JsonValue>;
    } catch{
      return {};
    }
  }
  if (coach.data && typeof coach.data === "object") return coach.data as Record<string, JsonValue>;
  return {};
}
...
export default function useCoaches(){
  ...
  const saveCoach = useCallback(
    async (formData: CoachForm, editingId: string | null = null) => {
      try{
