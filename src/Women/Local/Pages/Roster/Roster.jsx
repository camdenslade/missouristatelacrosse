// src/Women/Local/Pages/Roster/Roster.jsx
...
export default function WRoster({ userRole }){
  ...
  const handleDelete = async (item, coach) => {
    ...
    try{
      await deleteDoc(doc(db, coach ? "coachesw" : "playersw", item.id));
      await Promise.all([fetchPlayers(), fetchCoaches()]);
    } catch (err){
  ...
  return (
    <div className="max-w-full px-4 py-8">
      ...
      <div className="no-print flex flex-col sm:flex-row justify-between items-center mb-6">
        ...
        <button
          onClick={handlePrint}
          className="print:hidden mt-4 sm:mt-0 bg-gray-100 hover:bg-gray-200 text-gray-800 border border-gray-300 rounded-md px-4 py-2 text-sm font-medium"
        >
          Print Roster
        </button>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="hidden print:block roster-printable">
            ...
            <h1>{displaySeasonLabel(selectedSeason)} Women's Lacrosse Roster</h1>
            <table>
              <thead>
                <tr>
