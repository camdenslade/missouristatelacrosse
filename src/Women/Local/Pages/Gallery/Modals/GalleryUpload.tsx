...
export default function GalleryUploadModal({ onClose, onUpload }){
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedGameId && customFolder) {
      const folderError = validateFolderName(customFolder, "Folder name");
      if (folderError) return dispatch({ type: "ERROR", error: folderError });
    }
    const folder =
      selectedGameId && games.length
        ...
        : customFolder || "Misc";
    const folderSanity = validateText(folder, "Folder name", { required: true, max: 80 });
    if (folderSanity) return dispatch({ type: "ERROR", error: folderSanity });
