...
/**
 * Store checkout. Delegates the payment UI to usePaymentButtons (PayPal or Stripe per
 * VITE_PAYMENT_PROVIDER); the post-payment fulfilment (Printify order + receipt email +
 * cart clear + navigate) runs from the shared onSuccess callback, so it is identical
 * whichever rail was used. `finalTotal` must already include the shipping fee.
 */
export default function useStore(
  ...
  const onSuccess = useCallback(
    async (captureData: { id: string }) => {
      ...
      if (navigate) {
        navigate("/checkout-success", { state: { order: captureData } });
      }
    },
